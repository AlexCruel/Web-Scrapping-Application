...
class Site():
    ...
    def get_content(self, html, arg, select_class, find_str):
        ...
        cards.append(
            {
                'price': testItem.split('Цена:')[1].split(' руб')[0]
            }
        )
        print(testItem.split('Цена:')[1].split(' руб')[0] + ' --- ' + self.URL)
        return cards

    def save_document(self, testItem, location):
        workbook = openpyxl.load_workbook('../АКБ ZUBR в РБ.xlsx')
        workbook.active = 0
        ...
        workbook.save('../АКБ ZUBR в РБ.xlsx')
